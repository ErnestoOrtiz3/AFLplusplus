/*
 * AFL++ CPU Scheduler - Enhanced BPF Loader
 *
...
#include "afl_sched_enhanced.skel.h"
...
static struct afl_sched_enhanced_bpf *skel = NULL;
...
int main(int argc, char *argv[]) {
    ...
    skel = afl_sched_enhanced_bpf__open();
    if (!skel) {
    ...
    err = afl_sched_enhanced_bpf__load(skel);
    if (err) {
    ...
    err = afl_sched_enhanced_bpf__attach(skel);
    if (err) {
    ...
    printf("AFL++ Enhanced CPU Scheduler loaded successfully\n");
...
static void cleanup(void) {
    ...
    if (skel) {
        afl_sched_enhanced_bpf__destroy(skel);
    }
    
    printf("AFL++ Enhanced CPU Scheduler shut down\n");
}
