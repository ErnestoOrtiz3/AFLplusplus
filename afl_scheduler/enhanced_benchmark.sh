#!/bin/bash
#
# Script to benchmark the enhanced AFL++ CPU Scheduler
#
...
TEST_ID="enhanced_$(date +%Y%m%d_%H%M%S)"
RESULTS_DIR="enhanced_benchmark_results/${TEST_ID}"
...
run_benchmark() {
    ...
    if [ "$scheduler" = "custom" ]; then
        ...
        # Compile the enhanced scheduler if needed
        if [ ! -f "afl_scheduler/afl_sched_enhanced.skel.h" ]; then
            echo "Compiling enhanced scheduler..."
            cd afl_scheduler
            clang -g -O2 -target bpf -D__TARGET_ARCH_x86 \
                -I../scx -I../scx/scheds/include \
                -c bpf/afl_sched_enhanced.bpf.c -o bpf/afl_sched_enhanced.bpf.o
            llvm-strip -g bpf/afl_sched_enhanced.bpf.o
            bpftool gen skeleton bpf/afl_sched_enhanced.bpf.o > afl_sched_enhanced.skel.h
            gcc -Wall -Werror -O2 -o afl_sched_loader_enhanced afl_sched_loader_enhanced.c -lbpf -lelf -lm
            cd ..
        fi
        
        # Load the enhanced scheduler
        sudo afl_scheduler/afl_sched_loader_enhanced \
            -b "$BOOST_DURATION" \
            -w "$BOOST_WEIGHT" \
            -d "$BOOST_DECAY" \
            -s "$SLICE_US" \
        ...
        if ! ps aux | grep -q "[a]fl_sched_loader_enhanced"; then
    ...
    afl_scheduler/stats_collector "$output_dir" &
    STATS_PID=$!
    ...
    for i in $(seq 1 "$NUM_INSTANCES"); do
        ...
        # Start AFL++ instance
        sudo AFL_NO_AFFINITY=1 ./afl-fuzz -i original_seeds -o "$output_dir" \
    ...
    if [ "$scheduler" = "custom" ]; then
        ...
        sudo killall afl_sched_loader_enhanced
        sleep 2
    fi
...
python3 afl_scheduler/compare_results.py "$RESULTS_DIR/custom" "$RESULTS_DIR/cfs" > "$RESULTS_DIR/comparison_report.txt"
...
cp "$RESULTS_DIR/comparison_report.txt" "enhanced_benchmark_latest.txt"
echo "Latest comparison report also available at: enhanced_benchmark_latest.txt"
