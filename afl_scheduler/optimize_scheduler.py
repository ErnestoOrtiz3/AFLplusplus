...
def calculate_score(metrics, weights=None):
    ...
    if weights is None:
        weights = {
            'crashes_diff': 10.0,  # Highest weight for crashes
            'paths_diff': 5.0,     # High weight for paths
            'bitmap_diff': 3.0,    # Medium weight for coverage
            'edges_diff': 2.0,     # Medium weight for edges
            'execs_diff': 1.0      # Lower weight for speed
        }
