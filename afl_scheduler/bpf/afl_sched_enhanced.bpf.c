...
/*
 * AFL++ CPU Scheduler - Enhanced BPF Component
 *
...
/* Calculate square root approximation for non-linear weight scaling */
static u64 int_sqrt(u64 x)
{
    u64 res = 0;
    u64 bit = 1ULL << 30; // Highest possible bit for 32-bit result
    
    // Find the highest bit that fits
    while (bit > x)
        bit >>= 2;
    
    // Calculate square root using binary method
    while (bit != 0) {
        if (x >= res + bit) {
            x -= res + bit;
            res = (res >> 1) + bit;
...
static u64 get_task_weight(struct task_struct *p)
{
    ...
    if (is_afl_process(p)) {
        ...
        if (weight) {
            ...
            // Apply non-linear scaling to make weight differences more impactful
            // Using square root to make higher weights even more effective
            if (effective_weight > 100) {
                u64 excess = effective_weight - 100;
                u64 sqrt_excess = int_sqrt(excess * 100); // Scale up for precision
                effective_weight = 100 + sqrt_excess;
            }
...
static void update_stats(u64 dispatches, u64 afl_dispatches, u64 boosted_dispatches, bool is_enqueue)
{
    u32 key = 0;
    struct {
        u64 dispatches;
    ...
    if (is_enqueue) {
        stats_p->total_enqueues++;
        if (afl_dispatches > 0) {
            stats_p->total_afl_enqueues++;
        }
    }
}
...
s32 BPF_STRUCT_OPS(afl_sched_tick, struct task_struct *p)
{
    ...
    // Use non-linear scaling for more effective prioritization
    if (weight > 100) {
        // For high-weight tasks, accumulate vruntime more slowly
        task_context->vruntime += delta * 100 / weight;
    } else {
        // For normal or low-weight tasks, use standard calculation
        task_context->vruntime += delta * 100 / weight;
    }
...
s32 BPF_STRUCT_OPS(afl_sched_enqueue, struct task_struct *p, u64 enq_flags)
{
    ...
    if (!is_afl_process_fast(p)) {
        ...
        update_stats(0, 0, 0, true); // Update enqueue stats
        return 0;
    }
    ...
    if (bpf_probe_read_kernel(&pid, sizeof(pid), &p->pid)) {
        ...
        if (boost_until && *boost_until > now) {
            ...
            // Normal slice based on weight with non-linear scaling
            ...
            // Calculate slice with non-linear scaling for more effective prioritization
            if (weight > 100) {
                // For high-weight tasks, give more CPU time
                u64 weight_factor = 100 + int_sqrt((weight - 100) * 100);
                slice = slice_us * weight_factor / 100;
            } else {
                // For normal or low-weight tasks, use standard calculation
    ...
    // Update statistics
    update_stats(1, 1, is_boosted ? 1 : 0, true);
