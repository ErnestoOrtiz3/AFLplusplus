...
/*
 * AFL++ CPU Scheduler - BPF Component with Configurable Parameters
 *
...
static u64 get_task_weight(struct task_struct *p)
{
    ...
    if (is_afl_process(p)) {
        ...
        if (boost_until && *boost_until > now) {
            ...
            return boost_weight;  // Use configurable boost weight
        }
